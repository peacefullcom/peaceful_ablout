        ...
        @csrf
         ...
         <div class="col-sm-8 offset-sm-2 col-xs-12">        
            <div class="form-group row">
              ...
              <input class="col-sm-9 col-xs-12 form-control" id="code" name="code" type="text" value="{{ $votePlayer->code }}">
            </div>
            <div class="form-group row">
              <label for="name" class="col-sm-3 col-xs-12 required">name</label>
              <input class="col-sm-9 col-xs-12 form-control" id="name" name="name" type="text" value="{{ $votePlayer->name }}">
            </div>
            <div class="form-group row">
              <label for="content" class="col-sm-3 col-xs-12 required">content</label>
              <textarea class="col-sm-9 col-xs-12 form-control" id="content" name="content" rows="4" cols="50">{{ $votePlayer->content }}</textarea>
            </div>
            <div class="form-group row">
              <label for="image" class="col-sm-3 col-xs-12 required">image</label>
              <img class="img-fluid" src="/images/{{ $votePlayer->img }}" alt="Photo">
            ...
            <div class="form-group row">
              ...
              <select class="col-sm-9 col-xs-12 form-control" id="group_id" name="group_id">
                @foreach ($voteGroup as $k => $v)
                  <option value="{{ $k }}" @if($votePlayer->group_id == $k) selected @endif >{{ $v }}</option>
                @endforeach
              </select>
            </div>
            @endif
            <div class="form-group row">
              ...
              <input class="col-sm-9 col-xs-12 form-control" id="sort" name="sort" type="text" value="{{ $votePlayer->sort }}">
            </div>
            <div class="form-group row">
              <label for="is_active" class="col-sm-3 col-xs-12 required">Enabled</label>
              <input type="checkbox" name="is_active" data-bootstrap-switch data-off-color="danger" data-on-color="success" 
              @if($votePlayer->is_active == 1) checked @endif >
            </div>
            <div class="form-group row">
